...
def mem_spk_plot(data,times,sender,subplotspec,mem_color,spk_inh_color,spk_exc_color):
    ...
    if in_neurons.size>0 and ex_neurons.size>0:
        ...
        ax0.plot(times[sender==in_idx-1],sender[sender==in_idx-1]*0,color=spk_inh_color,marker='*',markersize=10,linestyle=' ')
        ax1.plot(t[id == ex_idx], v[id == ex_idx], mem_color, linewidth=0.8)
        ax1.plot(times[sender == ex_idx-1], sender[sender == ex_idx-1] * 0,color=spk_exc_color ,marker='*', markersize=10,linestyle=' ')
        for ax in [ax0,ax1]:

...
int main(int argc, char *argv[])
{
  ...
  double ratio_ex_ex = std::atof(argv[3]); // ratio of strong connections 
  double ratio_ex_inh = std::atof(argv[4]);
  ...
  for (int c = 0; c < json_in_data.size(); ++c)
  {
    ...
    // in this version of the group finding algorithm, we choose random connectivity with
    // variing the ratio between strong and weak synapses.
    
    if (source < Ne) // only ex -> [ex, inh] connections
    {
        double r;
        if (target < Ne){
            // ex->ex

...
exc_conns = np.array([final_stdw[i] for i in range(len(final_stdw)) if final_stdw[i]['weight'] >= 0.])

#randomly pick synapses with given probability
exc_conns = np.random.choice(exc_conns, int(len(exc_conns) * args.random), replace=False)

...
def build_simulate(stim_target_gids, stim_times, stim_weights, stim_delay, group, t_fired):
    import nest
...
#                }
                        ...
                        #print "new link", t, group[j], group[i], delay, layer[i]
...
#                 }
        ...
        #        else:
        #            delays = inh_delay[np.where(np.all([inh_pre == group[j], inh_post == groupt[i]], axis=0))[0]]
        #        for d in delays:
        #            layer = 2
        #            if links:
...
def worker(pivot_neuron):
    ...
    #print("pivot neuron", pivot_neuron)
    ...
    for stim_triplet in itertools.combinations(inc_exc_conns, 3):
       ...
       # if not stim_triplet[0]['pre'] in [5, 32, 63]:
       #     continue
       # if not stim_triplet[1]['pre'] in [5, 32, 63]:
       #     continue
       # if not stim_triplet[2]['pre'] in [5, 32, 63]:
       #     continue

        #print(" STIM TRIP", stim_triplet)
        stim_target_gids = []
        stim_times = []
        stim_weights = []
        ...
        json_group = build_simulate(np.array(stim_target_gids), np.array(
            stim_times), np.array(stim_weights), np.array(sorted_stim_delay), sorted_group, sorted_t_fired)
...
for found_groups in pool.imap_unordered(worker, range(1, Ne)):
